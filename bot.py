...
async def handle_lecture_action(action, session, understanding, ai_data=None):
    async with _action_lock:
        ...
        elif action in ["TEACH_NEXT_CHUNK", "ANSWER_AND_TEACH", "ANSWER_AND_CONTINUE", "INTRODUCE_LECTURE"]: # 👈 تمت إضافة الأكشن الجديد هنا ليزيد رقم الشريحة
            session["current_chunk_index"] += 1
        ...
        # 2. منطق إرسال الصور
        transition_actions = ["INTRODUCE_LECTURE", "TEACH_NEXT_CHUNK", "EVALUATE_AND_CONTINUE", "ANSWER_AND_CONTINUE", " ANSWER_AND_TEACH"]
...
# تشغيل Pyrogram
#  bot_app.run()
# أعلى الملف كمتغير عالمي
_started = False

async def start_all():
    global _started
    if _started:
        print("⚠️ start_all() already ran — skipping duplicate start.", flush=True)
        return
    _started = True
